...
queryFunction('SELECT paper_intl_conf.id, paper_status.name_eng as paper_name_eng, paper_status.name_kor as paper_name_kor, paper_conf_presentation.name_eng, paper_conf_presentation.name_kor, project.subject, research.title as research_title, conf_country, conf_city, conf_startdate, conf_enddate, conf_lncs, conf_name, author1, author2, author3, author4, author5, author6, author7, author8, paper_intl_conf.title, year, month, volumn, startpage, endpage, paper_file, coverpage_file, listpage_file, firstpage_file FROM paper_intl_conf LEFT JOIN paper_status ON paper_intl_conf.statusid = paper_status.id LEFT JOIN paper_conf_presentation ON paper_intl_conf.presentationid = paper_conf_presentation.id LEFT JOIN project ON paper_intl_conf.projectid = project.id LEFT JOIN research ON paper_intl_conf.researchid = research.id')
  .then(rows => {
    // console.log(rows);
    var i = 0;
    var dateArr = [];
    var pathPrefix = "/assets/files/publication/international/conference/";
    _.forEach(rows, row => {
      console.log(row);
      ...
      resultStr += "categories: international conference\n";
      ...
      (row.conf_country) ? 
      resultStr += "  country: \""+row.conf_country+"\"\n" : resultStr += "  country:\n"; 
      (row.conf_city) ?
      ...
      resultStr += "{% endif %}\n";
      resultStr += "{% if page.conference.city and page.conference.country %}\n";
      resultStr += "{{ page.conference.city }}, {{ page.conference.country }}\n\n";
      ...
      makeFileFunction(resultStr, "international/conference/"+finalDate+"-"+
